...
from forward_model_phase_params_johnathon import xuv_taylor_to_E, ir_from_params, A_fun
...
xuv_coefs_batch=np.random.uniform(low=-1.0,high=1.0,size=(1100,1,5))
ir_values_batch=np.random.uniform(low=-1.0,high=1.0,size=(1100,1,4))
...
xuv_shift=np.zeros((1100,10,1))
#ir_shift=ir_values_batch[:,:,0:1]
xuv_coefs_mod_batch=np.concatenate((xuv_shift,xuv_coefs_batch[:,:,1:5]),axis=2)
#ir_values_mod_batch=np.concatenate((ir_shift,ir_values_batch[:,:,1:4]),axis=2)
xuv_coefs_batch=np.reshape(xuv_coefs_mod_batch,(-1,5))
ir_values_batch=np.reshape(ir_values_batch,(-1,4))
...
xuv_pulse_train = sess.run(xuv_E_prop['f_cropped'], feed_dict=feed_dict_train)
ir_pulse_train = sess.run(ir_E_prop['E_prop']['f_cropped'], feed_dict=feed_dict_train)
xuv_pulse_test = sess.run(xuv_E_prop['f_cropped'], feed_dict=feed_dict_test)
ir_pulse_test = sess.run(ir_E_prop['E_prop']['f_cropped'], feed_dict=feed_dict_test)
...
#counts_train=np.kron(np.ones(1000,),np.array([20,30,40,50,60,70,80,90,100,110]))
#counts_test=np.kron(np.ones(100,),np.array([20,30,40,50,60,70,80,90,100,110]))
counts_train=np.kron(np.ones(1000,),np.array([10,10,32.5,32.5,55,55,77.5,77.5,100,100]))
#counts_test=np.kron(np.ones(100,),np.array([2,3,5,10,12,16,32,60,80,100]))
counts_test=np.kron(np.ones(100,),np.array([5,7,10,15,21,32.5,43,55,77.5,100]))
#counts_train=10*np.ones(10000)
#counts_test=10*np.ones(1000)
...
filename='/media/HDD1/zyz/FROG_train_Poisson/XUV_IR_train_Poisson_johnathon_sample4_mixed_10-100_myA.mat'
savemat(filename,\
...
filename='/media/HDD1/zyz/FROG_train_Poisson/XUV_IR_test_Poisson_johnathon_sample4_mixed_5-100_myA.mat'
savemat(filename,\
...
load=hdf5storage.loadmat('/media/HDD1/zyz/FROG_train_Poisson/XUV_IR_train_Poisson_johnathon_sample4_mixed_10-100_myA.mat')
...
load=hdf5storage.loadmat('/media/HDD1/zyz/FROG_train_Poisson/XUV_IR_test_Poisson_johnathon_sample4_mixed_10-100_myA.mat')
